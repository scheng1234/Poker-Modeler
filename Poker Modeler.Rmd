...
levels(Suits) <- c(1,2,3,4)

Deck <- expand_grid(Card_Values, Suits) %>% 
  mutate(., Card = paste(Number, Suits, sep=""))

# Deck <-
#   cross_df(Card_Values$Number, Suits) %>% 
#   sapply(., rbind) %>%  
#   as_tibble()
# 
...
to convert between the holdem package and modeler notation. 
